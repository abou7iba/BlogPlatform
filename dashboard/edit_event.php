...
if(isset($_POST['update_article'])){
    ...
    $add_courses = $conn->prepare("UPDATE `events` SET title = ?, event = ? WHERE id = ?");
...
<div class="modal-content">
        <?php
    $select_courses = $conn->prepare("SELECT * FROM `events` WHERE id = ? ");
    $select_courses->execute([$get_id]);
    if($select_courses->rowCount() > 0){
        while($fetch_course = $select_courses->fetch(PDO::FETCH_ASSOC)){
            ?>
            <br><br>
            <h2> تعديل الفاعليه </h2>
...
<div class="col">
      ...
      <p>   الفاعليه  <span>*</span></p>
      <input type="text" name="news" maxlength="100" value="<?= $fetch_course['event']; ?>"  class="box">
   </div>
