...
if(isset($_POST['update_article'])){
    ...
    $add_courses = $conn->prepare("UPDATE `articles` SET title = ?, article = ? WHERE id = ?");
    $add_courses->execute([ $title, $article , $get_id]);
}   
?>
    <style>
        body
...
<div class="modal-content">
        <?php
    $select_courses = $conn->prepare("SELECT * FROM `articles` WHERE id = ? ");
...
<div class="col">
      ...
      <input type="text" name="article" maxlength="100" value="<?= $fetch_course['article']; ?>"  class="box">
   </div>
